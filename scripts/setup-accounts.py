...
###### KEEP THE FOLLOWING UPDATED WITH THE VARIOUS COUNTRIES WE SUPPORT ######

# These should match the `EXAMPLE_DEMO_ONBOARDING_ACCOUNT*` environment variables
DEMO_ONBOARDING_COUNTRIES = ["GB", "US", "FR", "SG", "HK", "AU"]
...
DEFAULT_CAPABILITIES = ["card_payments", "transfers"]

CAPABILITIES = {
    "US": DEFAULT_CAPABILITIES + ["card_issuing", "treasury"],
    "GB": DEFAULT_CAPABILITIES + ["card_issuing"],
}
...
# Also add to lib/auth.ts
TEST_EXTERNAL_ACCOUNTS = {
    "US": {
...
# This needs to be a real email domain so that support tickets work
...
def has_capability(account, capability):
    assert isinstance(account, stripe.Account)

    return account.capabilities.get(capability, None) == "active"
...
def ensure_accounts(country, create=False):
    """
    ...
    platform = stripe.Account.retrieve()
    ...
    for acct_country in countries_to_create:
        log.info(f"Creating a demo onboarding account for {acct_country}")

        # Only support non-standard capabilities if the account is in the same country as the platform
        capabilities = (
            CAPABILITIES.get(acct_country, DEFAULT_CAPABILITIES)
            if acct_country == platform_country
        ...
        account = stripe.Account.create(
            ...
            capabilities=dict([c, {"requested": True}] for c in capabilities),
            metadata={
                PROTECTED_TAG: "true",
        ...
        log.info(f"*********************")
        log.info(
            f"Created account {account.id} as a demo onboarding account for country {acct_country}"
        )
        log.info(f"*********************")
    ...
    for i in range(to_create):
        ...
        bank_account_data = {
            ...
            **TEST_EXTERNAL_ACCOUNTS[country],
        }
        ...
        capabilities = (
            CAPABILITIES.get(country, DEFAULT_CAPABILITIES)
        ...
        issuing_tos = (
            ...
            if "card_issuing" in capabilities
            else {}
        )
        ...
        stripe.Account.create(
            ...
            business_profile={
                ...
                "support_address": SUPPORT_ADDRESSES[country],
                "support_email": identity["email"],
                "support_phone": PHONE_NUMBERS[country],
            ...
            individual={
                **{
                    "address": VALIDATED_ADDRESSES[country],
                    "dob": {
                        "day": 1,
                        "month": 1,
                        "year": 1901,
                    ...
                    "id_number": "222222222",
                    "phone": PHONE_NUMBERS[country],
                },
                **({"ssn_last_4": "2222"} if country == "US" else {}),
            },
            metadata={},
            settings={
                **{
...
def ensure_financial_account(account):
    ...
    if is_rejected_account(account) or not has_capability(account, "card_payments"):
        return

    if not has_capability(account, "treasury"):
        return
    ...
    stripe.treasury.FinancialAccount.create(
        supported_currencies=[CURRENCIES[account.country]],
        features={
...
def rebalance_account_statuses():
    ...
    if len(demo_onboarding_accounts) < len(DEMO_ONBOARDING_COUNTRIES):
        ...
        for account in demo_onboarding_accounts:
            log.info(f"Demo account for {account.country} found: {account.id}")
    ...
    if is_restricted_account(account):
        ...
        stripe.Account.modify(
            ...
            business_profile={
                ...
                "support_address": {
                    **SUPPORT_ADDRESSES[account.country],
                    **{
...
def create_charge(account, payment_method):
    ...
    try:
        pi = stripe.PaymentIntent.create(
            ...
            currency=CURRENCIES[account.country],
...
def generate_payouts(account):
    ...
    if not external_accounts:
        log.info(f"No external accounts for {account.id}")
        return

    currency = external_accounts[0].currency
...
def create_cardholder_and_card(account, financial_account_id=None):
    assert isinstance(account, stripe.Account)

    if not has_capability(account, "card_issuing"):
        return None
    ...
    try:
        cardholder = stripe.issuing.Cardholder.create(
            ...
            email=f"{first_name.lower()}.{last_name.lower()}@example.com",
            phone_number=PHONE_NUMBERS[account.country],
            billing={
                "address": SUPPORT_ADDRESSES[account.country],
            },
            individual={
                "first_name": first_name,
                "last_name": last_name,
...
def generate_cardholders_and_cards(account):
    ...
    use_treasury = has_capability(account, "treasury")
    ...
    # Get the existing cards on the first page
    cards = list(
        sorted(
    ...
    if index == -1 and cards:
        log.info(
            f"No inactive cards on the first page for {account.id}. Not generating more."
        )
        return

    cards_to_create = 10 - index

    log.info(f"Generating {cards_to_create} cardholders and cards for {account.id}")

    financial_account = get_default_financial_account(account) if use_treasury else None

    financial_account_id = financial_account.id if financial_account else None

    for _ in range(cards_to_create):
        create_cardholder_and_card(account, financial_account_id=financial_account_id)
...
def generate_financial_account_transactions(account):
    ...
    if not has_capability(account, "card_issuing") or not has_capability(
        account, "treasury"
    ):
        return None
...
def generate_support_ticket(account, skip_existing=True):
    ...
    if not account.email:
        log.info(f"Skipping support ticket for account {account.id} as it has no email")
        return
...
def main(
    ...
    if not env_file:
        raise ValueError("No environment file specified")

    log.info(f"Running with env file {env_file}")

    env_file_location = os.path.join(ROOT_DIR, env_file if env_file else ".env.local")
    if not os.path.exists(os.path.join(ROOT_DIR, env_file)):
        raise ValueError(
            f"Environment file {env_file} does not exist at {env_file_location}"
        )

    config = dotenv_values(env_file_location)
    ...
    if "NEXT_PUBLIC_DEFAULT_COUNTRY" not in config:
        raise ValueError(
            "NEXT_PUBLIC_DEFAULT_COUNTRY is not defined in the environment"
        )
    country = config["NEXT_PUBLIC_DEFAULT_COUNTRY"]
    ...
    ensure_accounts(country, create=create_accounts)
    ...
    # Ensure our demo account is included here too
    demo_account_id = config["EXAMPLE_DEMO_ACCOUNT"]
    if demo_account_id:
        demo_account = stripe.Account.retrieve(demo_account_id)
        if demo_account not in accounts:
            log.info(
                f"Adding demo account {demo_account.id} to the list of recent accounts"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env-file",
        help="Use the a specific .env file",
        type=str,
        default=".env.local",
        dest="env_file",
    )
