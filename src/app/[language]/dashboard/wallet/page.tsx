  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 360px', gap: 20, alignItems: 'start' }}>
      ...
      <div>
        ...
        {/* AI prompt bar */}
        <div style={{
          background: '#f3f8e9', border: '1px solid #c9dca0', borderRadius: 8,
          padding: '14px 16px', marginBottom: 14,
          display: 'flex', alignItems: 'center', gap: 12,
        }}>
          <span style={{ width: 36, height: 36, borderRadius: 999, background: '#77B32A', color: '#fff', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <IMsg />
          </span>
          <input
            value={aiInput}
            onChange={e => setAiInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') onAsk(aiInput); }}
            placeholder="What would you like to do? e.g. 'Move £800 to my VAT pot'"
            style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: 14, color: '#323E48', minWidth: 0 }}
          />
          <button
            onClick={() => onAsk(aiInput)}
            style={{ background: '#77B32A', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
          >
            <ISend /> Send
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' as const }}>
          {SUGGESTIONS.map((s, i) => (
            <button key={i} onClick={() => onAsk(s)} style={{ background: '#fff', border: '1px solid #D8DCE0', color: '#4D5761', borderRadius: 999, padding: '5px 12px', fontSize: 12, cursor: 'pointer' }}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div style={{ background: '#FAFBFC', border: '1px solid #EEF0F2', borderRadius: 10, padding: '16px 18px 18px', marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, paddingBottom: 10, borderBottom: '1px dashed #E1E4E8' }}>
            <span style={{ width: 22, height: 22, borderRadius: 999, background: '#eaf0f6', color: '#4D5761', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              <IMsg />
            </span>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#4D5761', letterSpacing: '.08em', textTransform: 'uppercase' as const }}>Conversation history</span>
            <span style={{ marginLeft: 'auto', fontSize: 11, color: '#8892A0' }}>{convo.length} message{convo.length === 1 ? '' : 's'}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
            {convo.length === 0 && (
              <div style={{ fontSize: 12, color: '#8892A0', padding: '8px 4px', fontStyle: 'italic' }}>Ask the assistant something above to get started.</div>
            )}
            {convo.map((c, i) =>
              c.kind === 'user' ? (
                <div key={i} style={{ alignSelf: 'flex-end', maxWidth: 540, background: '#323E48', color: '#fff', padding: '10px 14px', borderRadius: '14px 14px 4px 14px', fontSize: 14, lineHeight: 1.45 }}>
                  {c.text}
                </div>
              ) : null
            )}
            ...
            {pending && (
              <div style={{
                background: '#fff', border: '1px solid #D8DCE0', borderLeft: '3px solid #77B32A',
                borderRadius: 8, padding: '18px 20px', boxShadow: '0 2px 8px rgba(0,0,0,.05)',
                maxWidth: 620, alignSelf: 'flex-start',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <span style={{ width: 30, height: 30, borderRadius: 999, background: 'linear-gradient(135deg, #77B32A, #5a881f)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <ISparkle />
                  </span>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#323E48' }}>
                    Wallet Assistant{' '}
                    <span style={{ background: '#f3f8e9', color: '#5a881f', fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 999, marginLeft: 6, letterSpacing: '.04em', textTransform: 'uppercase' as const }}>AI</span>
                  </span>
                </div>
                <div style={{ fontSize: 14, color: '#323E48', lineHeight: 1.55, marginBottom: 14 }}>
                  I'll move{' '}
                  <span style={{ background: '#f3f8e9', padding: '1px 5px', borderRadius: 3, color: '#3f5e16', fontWeight: 600 }}>
                    £{pending.amount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                  {' '}from your <b>Available balance</b> into your <b>{pending.target}</b>. This will earn <b>{pending.aer}% AER</b>. Your new available balance will be <b>£{(localAvailable - pending.amount).toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b>. Shall I go ahead?
                </div>
                <div style={{ background: '#FAFBFC', border: '1px solid #EEF0F2', borderRadius: 6, padding: '12px 14px', marginBottom: 14, display: 'grid', gridTemplateColumns: '1fr auto', gap: '6px 16px', fontSize: 13 }}>
                  <span style={{ color: '#4D5761' }}>Available balance now</span>
                  <span style={{ color: '#323E48', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>£{localAvailable.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  <span style={{ color: '#4D5761' }}>→ Move to {pending.target}</span>
                  <span style={{ color: '#5a881f', fontWeight: 600 }}>−£{pending.amount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  <span style={{ color: '#323E48', fontWeight: 600, borderTop: '1px solid #D8DCE0', paddingTop: 6, marginTop: 4 }}>New available balance</span>
                  <span style={{ color: '#323E48', fontWeight: 600, fontVariantNumeric: 'tabular-nums', borderTop: '1px solid #D8DCE0', paddingTop: 6, marginTop: 4 }}>£{(localAvailable - pending.amount).toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#5a881f', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
                    Edit details
                  </button>
                </div>
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', margin: '8px 0 12px' }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#323E48', margin: 0, letterSpacing: '-0.01em' }}>Your pots</h2>
          <span style={{ fontSize: 13, color: '#5a881f', fontWeight: 600, cursor: 'pointer' }}>Manage allocations →</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 16, marginBottom: 22 }}>
          {pots.map((p, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 6, boxShadow: '0 2px 8px rgba(0,0,0,.08)', padding: '22px 24px', position: 'relative', overflow: 'hidden', borderLeft: `4px solid ${p.tone === 'navy' ? '#323E48' : '#77B32A'}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
                <div style={{ width: 38, height: 38, borderRadius: 8, background: p.tone === 'navy' ? '#eaf0f6' : '#f3f8e9', color: p.tone === 'navy' ? '#323E48' : '#5a881f', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {p.tone === 'navy' ? <IShield /> : <IVault />}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ fontSize: 16, fontWeight: 700, color: '#323E48', letterSpacing: '-0.01em', margin: 0, lineHeight: 1.2 }}>{p.name}</h3>
                  <div style={{ fontSize: 12, color: '#8892A0', marginTop: 2 }}>{p.purpose}</div>
                </div>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: p.tone === 'navy' ? '#eaf0f6' : '#f3f8e9', color: p.tone === 'navy' ? '#3f5673' : '#5a881f', fontSize: 11, fontWeight: 700, letterSpacing: '.04em', textTransform: 'uppercase' as const, padding: '3px 8px', borderRadius: 999 }}>
                  {p.aer}% AER
                </span>
              </div>
              ...
              <div style={{ height: 6, background: '#F4F4F4', borderRadius: 999, marginTop: 12, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: p.tone === 'navy' ? '#323E48' : '#77B32A', borderRadius: 999, width: `${p.progress}%` }} />
              </div>
              <div style={{ fontSize: 11, color: '#8892A0', marginTop: 6, display: 'flex', justifyContent: 'space-between' }}>
                <span>{p.progressLabel}</span>
                <span><b style={{ color: '#323E48' }}>{p.progress}%</b> of target</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14, paddingTop: 14, borderTop: '1px solid #F4F4F4', fontSize: 12, color: '#4D5761' }}>
                <span>{p.tone === 'navy' ? 'Interest accrued' : 'Interest-bearing'} · this month</span>
                <b style={{ color: '#323E48' }}>+£{p.interest.toFixed(2)}</b>
              </div>
              <div style={{ display: 'flex', gap: 6, marginTop: 14 }}>
                {(['Top up', 'Withdraw', 'Edit'] as const).map((label, j) => (
                  <button key={label} style={{ padding: '7px 10px', fontSize: 12, borderRadius: 4, border: '1px solid #D8DCE0', background: 'transparent', color: '#4D5761', cursor: 'pointer', marginLeft: j === 2 ? 'auto' : undefined }}>
                    {label}
                  </button>
                ))}
              </div>
          ...
          <button style={{ background: '#fff', border: '2px dashed #D8DCE0', borderRadius: 6, padding: '22px 24px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', gap: 8, color: '#8892A0', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>
            <IPlus />
            Create a new pot
          </button>
          <div /> {/* spacer */}
